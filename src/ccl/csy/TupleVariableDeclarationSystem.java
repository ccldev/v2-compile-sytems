...
public class TupleVariableDeclarationSystem implements CompileSystem<CclCodeSnippet, File>{
	
	private static final Pattern VAR_PATTERN = Pattern.compile
			("\\s*var\\s+\\(([^=]+)\\)\\s*=\\s*(.+)\\s*;", Pattern.DOTALL);
	private static final Pattern PART_PATTERN = Pattern.compile
			("!?\\s*([a-zA-Z_][a-zA-Z0-9_]*)\\s*", Pattern.DOTALL);
	...
	public String compileComplete(CclCodeSnippet infos)
		...
		for(int i = 0; i < splitted.length; i++){
			b.append("\n");
			b.append(process(splitted[i].trim(), i));
		}
	...
	private String process(String s, int index) {
